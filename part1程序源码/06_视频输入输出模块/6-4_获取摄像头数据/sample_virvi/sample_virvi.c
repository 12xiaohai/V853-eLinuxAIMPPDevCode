...
static int ParseCmdLine(int argc, char **argv,
                        SampleVirViCmdLineParam *pCmdLinePara) {
...
static void judgeCaptureFormat(char *pFormatConf, PIXEL_FORMAT_E *pCapFromat) {
  if (NULL != pFormatConf) {
    ...
    /* yuv422 format */
    else if (!strcmp(pFormatConf, "nv16")) {
      *pCapFromat = MM_PIXEL_FORMAT_YUV_SEMIPLANAR_422;
    } else if (!strcmp(pFormatConf, "nv61")) {
      *pCapFromat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_422;
    }
    ...
    /* raw data format */
    else if (!strcmp(pFormatConf, "srggb8")) {
      *pCapFromat = MM_PIXEL_FORMAT_RAW_SRGGB8;
    } else if (!strcmp(pFormatConf, "srggb10")) {
      *pCapFromat = MM_PIXEL_FORMAT_RAW_SRGGB10;
    } else if (!strcmp(pFormatConf, "srggb12")) {
      *pCapFromat = MM_PIXEL_FORMAT_RAW_SRGGB12;
    } else {
      *pCapFromat = MM_PIXEL_FORMAT_YVU_PLANAR_420;
...
void judgeCaptureColorspace(char *pColorSpConf,
                            enum v4l2_colorspace *pV4L2ColorSp) {
  if (NULL != pColorSpConf) {
    if (!strcmp(pColorSpConf, "jpeg")) {
      ...
      *pV4L2ColorSp = V4L2_COLORSPACE_REC709_PART_RANGE;
      ...
      aloge("fatal error! wrong color space:%s, use dafault jpeg",
            pColorSpConf);
      *pV4L2ColorSp = V4L2_COLORSPACE_JPEG;
    }
  }
...
static ERRORTYPE loadConfigPara(SampleVirViContext *pContext,
  ...
  if (conf_path != NULL) {
    ...
    for (int i = 0; i < MAX_CAPTURE_NUM; i++) {
      ...
      pConfig->AutoTestCount = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Auto_Test_Count, i), 0);
      pConfig->GetFrameCount = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Get_Frame_Count, i), 0);
      pConfig->DevNum = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Dev_Num, i), 0);
      pConfig->mIspDevNum = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_ISP_Dev_Num, i), 0);
      pConfig->FrameRate = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Frame_Rate, i), 0);
      pConfig->PicWidth = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Pic_Width, i), 0);
      pConfig->PicHeight = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Pic_Height, i), 0);
      ptr = (char *)GetConfParaString(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Pic_Format, i), NULL);
      judgeCaptureFormat(ptr, &pConfig->PicFormat);
      ptr = (char *)GetConfParaString(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_COLOR_SPACE, i), NULL);
      judgeCaptureColorspace(ptr, &pConfig->mColorSpace);
      pConfig->mEnableWDRMode = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_Enable_WDR, i), 0);
      pConfig->mViDropFrmCnt = GetConfParaInt(
          &stConfParser, parserKeyCfg(SAMPLE_VirVi_ViDropFrmCnt, i), 0);

      alogd("capture[%d] dev num[%d], capture size[%dx%d] format[%d] "
            "framerate[%d] colorspace[%d] EnWDR[%d] drop frm[%d]",
            i, pConfig->DevNum, pConfig->PicWidth, pConfig->PicHeight,
            pConfig->PicFormat, pConfig->FrameRate, pConfig->mColorSpace,
            pConfig->mEnableWDRMode, pConfig->mViDropFrmCnt);
    }

    pSaveBufMgrConfig->mSavePicDev =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_SavePicDev, 0);
    pSaveBufMgrConfig->mYuvFrameCount =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_YuvFrameCount, 0);
    ptr = (char *)GetConfParaString(&stConfParser, SAMPLE_VirVi_YuvFilePath,
                                    NULL);
    if (ptr)
      strcpy(pSaveBufMgrConfig->mYuvFile, ptr);
    pSaveBufMgrConfig->mRawStoreCount =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_RawStoreCount, 0);
    pSaveBufMgrConfig->mRawStoreInterval =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_RawStoreInterval, 0);
    ptr = (char *)GetConfParaString(&stConfParser, SAMPLE_VirVi_StoreDir, NULL);
    if (ptr)
      strcpy(pSaveBufMgrConfig->mStoreDirectory, ptr);

    pSaveBufMgrConfig->mSavePicBufferNum =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_SavePicBufferNum, 0);
    pSaveBufMgrConfig->mSavePicBufferLen =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_SavePicBufferLen, 0);
    if (0 == pSaveBufMgrConfig->mSavePicBufferLen) {
      SampleVirViConfig *pSaveConfig = NULL;
      for (int i = 0; i < MAX_CAPTURE_NUM; i++) {
        pSaveConfig = &pContext->mCaps[i].mConfig;
        if (pSaveConfig->DevNum == pSaveBufMgrConfig->mSavePicDev)
          break;
      }
      if (pSaveConfig) {
        /* yuv422: nv16 and nv61 */
        if (MM_PIXEL_FORMAT_YUV_SEMIPLANAR_422 == pSaveConfig->PicFormat ||
            MM_PIXEL_FORMAT_YVU_SEMIPLANAR_422 == pSaveConfig->PicFormat) {
          pSaveBufMgrConfig->mSavePicBufferLen =
              pSaveConfig->PicWidth * pSaveConfig->PicHeight * 2;
        }
        /* aw_lbc1.0x is a lossless format that takes up more buf than the yuv
           format. */
        else if (MM_PIXEL_FORMAT_YUV_AW_LBC_1_0X == pSaveConfig->PicFormat) {
          /* estimate the size of the buf and allocate it to a larger value. */
          pSaveBufMgrConfig->mSavePicBufferLen =
              pSaveConfig->PicWidth * pSaveConfig->PicHeight * 2;
        } else {
          pSaveBufMgrConfig->mSavePicBufferLen =
              pSaveConfig->PicWidth * pSaveConfig->PicHeight * 3 / 2;
        }
      }
      alogd("user did not specify buf len. set a default value %d bytes",
            pSaveBufMgrConfig->mSavePicBufferLen);
    }

    pContext->mTestDuration =
        GetConfParaInt(&stConfParser, SAMPLE_VirVi_Test_Duration, 0);

    alogd("save pic dev[%d], yuv count[%d] file[%s], raw count[%d] "
          "interval[%d] save dir[%s], buf len[%d] num[%d]",
          pSaveBufMgrConfig->mSavePicDev, pSaveBufMgrConfig->mYuvFrameCount,
          pSaveBufMgrConfig->mYuvFile, pSaveBufMgrConfig->mRawStoreCount,
          pSaveBufMgrConfig->mRawStoreInterval,
          pSaveBufMgrConfig->mStoreDirectory,
          pSaveBufMgrConfig->mSavePicBufferLen,
          pSaveBufMgrConfig->mSavePicBufferNum);
...
static int saveRawData(SampleVirviSaveBufMgr *pSaveBufMgr,
                       SampleVirviSaveBufNode *pNode) {
  ...
  case MM_PIXEL_FORMAT_YUV_SEMIPLANAR_422: {
    strcpy(strPixFmt, "nv16");
    break;
  }
  case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_422: {
    strcpy(strPixFmt, "nv61");
    break;
  }
  ...
  snprintf(strFilePath, MAX_FILE_PATH_SIZE, "%s/%dx%d_%s_vipp%d_%d.%s",
           pConfig->mStoreDirectory, pNode->mFrmSize.Width,
           pNode->mFrmSize.Height, strPixFmt, pConfig->mSavePicDev,
           pNode->mFrmCnt, fileType);
...
static int copyVideoFrame(VIDEO_FRAME_INFO_S *pSrc,
                          SampleVirviSaveBufNode *pNode) {
  ...
  unsigned int dataHeight =
      pSrc->VFrame.mOffsetBottom - pSrc->VFrame.mOffsetTop;
  alogv("dataWidth:%d, dataHeight:%d", dataWidth, dataHeight);

  switch (pSrc->VFrame.mPixelFormat) {
  case MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420: // nv12
  case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420: // nv21
  case MM_PIXEL_FORMAT_AW_NV21M:
    nFrmsize[0] = dataWidth * dataHeight;
    nFrmsize[1] =
        dataWidth * dataHeight / 2; // 1/4u and 1/4v interleaved storage
    nFrmsize[2] = 0;
    break;
  case MM_PIXEL_FORMAT_YVU_PLANAR_420: // yv12
  case MM_PIXEL_FORMAT_YUV_PLANAR_420: // yu12
    nFrmsize[0] = dataWidth * dataHeight;
    nFrmsize[1] = dataWidth * dataHeight / 4;
    nFrmsize[2] = dataWidth * dataHeight / 4;
    break;
  case MM_PIXEL_FORMAT_YUV_SEMIPLANAR_422: // nv16
  case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_422: // nv61
    nFrmsize[0] = dataWidth * dataHeight;
    nFrmsize[1] = dataWidth * dataHeight; // 1/2u and 1/2v interleaved storage
    nFrmsize[2] = 0;
    break;
  case MM_PIXEL_FORMAT_YUV_AW_LBC_2_0X:
  case MM_PIXEL_FORMAT_YUV_AW_LBC_2_5X:
  ...
  case MM_PIXEL_FORMAT_RAW_SRGGB8:
    ...
    break;
  default:
    aloge("fatal error! not support pixel format[0x%x]",
          pSrc->VFrame.mPixelFormat);
    return -1;
  }

  if (pNode->mDataLen < (nFrmsize[0] + nFrmsize[1] + nFrmsize[2])) {
    aloge("fatal error! buffer len[%d] < frame len[%d]", pNode->mDataLen,
          nFrmsize[0] + nFrmsize[1] + nFrmsize[2]);
    return -1;
  }
...
static void *SaveCsiFrameThrad(void *pThreadData) {
  ...
  while (1) {
    ...
    SampleVirviSaveBufNode *pEntry = list_first_entry_or_null(
        &pBufMgr->mReadyList, SampleVirviSaveBufNode, mList);
    if (pEntry) {
      pthread_mutex_lock(&pBufMgr->mIdleListLock);
      if (pEntry->mFrmCnt <= pConfig->mYuvFrameCount) {
        if (fp) {
          alogv("save frm[%d] len[%d] file[%s] success!", pEntry->mFrmCnt,
                pEntry->mFrmLen, pConfig->mYuvFile);
...
static void *GetCSIFrameThread(void *pThreadData) {
  ...
  while (1) {
    ...
    // vi get frame
    if ((ret = AW_MPI_VI_GetFrame(pCap->Dev, pCap->Chn, &pCap->pstFrameInfo,
                                  pCap->s32MilliSec)) != 0) {
    ...
    if (frm_cnt % 60 == 0) {
      ...
      alogd("Cap threadid=0x%lx, Vi%d-%d, %dx%d, %d-%d-%d-%d, mpts=%lld; local "
            "time is %s",
            pCap->thid, pCap->Dev, pCap->Chn, pCap->pstFrameInfo.VFrame.mWidth,
            pCap->pstFrameInfo.VFrame.mHeight,
            pCap->pstFrameInfo.VFrame.mOffsetLeft,
            pCap->pstFrameInfo.VFrame.mOffsetRight,
            pCap->pstFrameInfo.VFrame.mOffsetBottom,
            pCap->pstFrameInfo.VFrame.mOffsetTop,
    ...
    if (NULL != pBufMgr) {
      if (pBufMgrConfig->mSavePicDev == pCap->Dev) {
        SampleVirviSaveBufNode *pEntry = list_first_entry_or_null(
            &pBufMgr->mIdleList, SampleVirviSaveBufNode, mList);
        if (pEntry) {
          pthread_mutex_lock(&pBufMgr->mIdleListLock);
...
static SampleVirviSaveBufMgr *initSaveBufMgr(SampleVirViContext *pContext) {
  ...
  memcpy(pConfig, &pContext->mSaveBufMgrConfig,
         sizeof(SampleVirviSaveBufMgrConfig));

  INIT_LIST_HEAD(&pSaveBufMgr->mIdleList);
  INIT_LIST_HEAD(&pSaveBufMgr->mReadyList);
  /* 多线程中要想操作链表，必须要先获得互斥锁 */
  ...
  for (int i = 0; i < pConfig->mSavePicBufferNum; i++) {
    ...
    AW_MPI_SYS_MmzAlloc_Cached(&pNode->mDataPhyAddr, &pNode->mpDataVirAddr,
                               pNode->mDataLen);
    ...
    alogd("node[%d] alloc data len[%d] phy addr[%d] vir addr[%p]", i,
          pNode->mDataLen, pNode->mDataPhyAddr, pNode->mpDataVirAddr);
  ...
  list_for_each_entry_safe(pEntry, pTmp, &pSaveBufMgr->mIdleList, mList) {
    ...
    if (0 != pEntry->mDataPhyAddr && NULL != pEntry->mpDataVirAddr) {
      alogd("node[%d] alloc data len[%d] phy addr[%d] vir addr[%p]",
            pEntry->mId, pEntry->mDataLen, pEntry->mDataPhyAddr,
            pEntry->mpDataVirAddr);
...
static void initCaps(SampleVirViContext *pContext) {
  ...
  for (int i = 0; i < MAX_CAPTURE_NUM; i++) {
    ...
    if ((pConfig->DevNum < 0) || (0 == pConfig->PicWidth) ||
        (0 == pConfig->PicHeight)) {
...
static void configViAttr(VI_ATTR_S *pViAttr, SampleVirViConfig *pConfig) {
  ...
  pViAttr->format.pixelformat =
      map_PIXEL_FORMAT_E_to_V4L2_PIX_FMT(pConfig->PicFormat);
  ...
  pViAttr->capturemode =
      V4L2_MODE_VIDEO; /* V4L2_MODE_VIDEO; V4L2_MODE_IMAGE; V4L2_MODE_PREVIEW */
  pViAttr->drop_frame_num =
      pConfig->mViDropFrmCnt; // drop 2 second video data, default=0
}
...
static int start(SampleVirviCap *pCap) {
  ...
  if (eRet) {
    aloge("fatal error! vi dev[%d] enable virvi chn[%d] fail!", pCap->Dev,
          pCap->Chn);
    return -1;
  }
...
int main(int argc, char *argv[]) {
  ...
  SampleVirViContext *pContext =
      (SampleVirViContext *)malloc(sizeof(SampleVirViContext));
  ...
  if (argc > 1) {
    /* 0、parse command line param,read sample_virvi.conf */
  ...
  if (pContext->mSaveBufMgrConfig.mSavePicDev >= 0 &&
    ...
    pContext->mpSaveBufMgr = initSaveBufMgr(pContext); // 4、初始化缓冲区管理器
    if (NULL == pContext->mpSaveBufMgr) {
      aloge("fatal error! init save csi frame mgr fail!");
      goto _deinit_buf_mgr;
    }
    pthread_create(&saveFrmTrd, NULL, SaveCsiFrameThrad,
                   (void *)pContext); // 5、创建保存图像线程
  }

  for (int i = 0; i < MAX_CAPTURE_NUM; i++) {
    pCap = &pContext->mCaps[i];
    ret = prepare(pCap); // 6、配置VI组件参数并启动
    if (ret) {

...
static int SampleG2d_G2dOpen(SAMPLE_G2D_CTX *p_g2d_ctx)//打开g2d设备节点
{
    int ret = 0;
...
static int SampleG2d_G2dClose(SAMPLE_G2D_CTX *p_g2d_ctx)//关闭g2d设备节点
{
    if(p_g2d_ctx->mG2dFd >= 0)
    {
...
static int SampleG2d_G2dConvert_rotate(SAMPLE_G2D_CTX *p_g2d_ctx)
{
    ...
    if(pConfig->mDstRotate >= 0){
        switch(pConfig->mDstRotate)//旋转参数
        {
            case 0:
    ...
    //blit.src_image_h.bbuff = 1; //图像的缓冲区编号
    //blit.src_image_h.color = 0xff; /存储图像的颜色信息
    blit.src_image_h.format = eSrcFormat;//图像格式
    blit.src_image_h.laddr[0] = (unsigned int)p_g2d_ctx->src_frm_info.p_phy_addr[0];//图像物理地址
    ...
    blit.src_image_h.width = p_g2d_ctx->src_frm_info.frm_width;//输入图像宽度
    blit.src_image_h.height = p_g2d_ctx->src_frm_info.frm_height;//输出图像高度
    blit.src_image_h.align[0] = 0; //每个缓冲区的内存对齐要求
    blit.src_image_h.align[1] = 0; 
    blit.src_image_h.align[2] = 0;
    blit.src_image_h.clip_rect.x = pConfig->mSrcRectX;//裁剪矩形位置和大小
    ...
    blit.src_image_h.gamut = G2D_BT601; //颜⾊空间
    blit.src_image_h.bpremul = 0; //是否进行预乘 alpha 处理
    //blit.src_image_h.alpha = 0xff;
    blit.src_image_h.mode = G2D_PIXEL_ALPHA;   //G2D_PIXEL_ALPHA, G2D_GLOBAL_ALPHA
    blit.src_image_h.fd = -1; //// 源图像文件描述符，这里设置为 -1 表示没有关联文件描述符
    blit.src_image_h.use_phy_addr = 1; //// 表示使用物理地址访问源图像数据，这里设置为使用物理地址
    ...
    ret = ioctl(p_g2d_ctx->mG2dFd, G2D_CMD_BITBLT_H, (unsigned long)&blit);//实现单幅图的缩放、格式转换等
    if(ret < 0)
    {
...
//            break;
//    }
    aloge("src format[0x%x] dst format[0x%x]",eSrcFormat,eDstFormat);
    //blit.src_image_h.bbuff = 1;
    //blit.src_image_h.color = 0xff;
    blit.src_image_h.format = eSrcFormat;
    blit.src_image_h.laddr[0] = (unsigned int)p_g2d_ctx->src_frm_info.p_phy_addr[0];
...
static int SampleG2d_G2dConvert(SAMPLE_G2D_CTX *p_g2d_ctx)
{
    ...
    {
        ret = SampleG2d_G2dConvert_rotate(p_g2d_ctx);//旋转功能
    }
    else if(1 == pConfig->g2d_mod)
    {
        ret = SampleG2d_G2dConvert_scale(p_g2d_ctx);//缩放功能
    }
    else if(2 == pConfig->g2d_mod)
    {
        ret = SampleG2d_G2dConvert_formatconversion(p_g2d_ctx);//格式转换功能，实现rgb和yuv格式转换
    }
    else if (3 == pConfig->g2d_mod)
    {
        ret = SampleG2d_G2dBld(p_g2d_ctx);// 透明叠加功能，实现两个rgb图片叠加
    }
    else if (4 == pConfig->g2d_mod)
    {
        ret = SampleG2d_G2dMixter_Task(p_g2d_ctx);//缩放批处理功能，实现对图像进行批处理缩放功能
    }
...
int main(int argc, char *argv[])
{
    ...
    g2d_MemOpen();//打开和初始化 ION 内存管理器

    if(pConfig->g2d_mod != 4)
    {
        ret = PrepareFrmBuff(p_g2d_ctx);//根据图像尺寸准备输入和输出的缓冲区
        if(0 != ret)
        {
        ...
        p_g2d_ctx->fd_in = fopen(p_g2d_ctx->mConfigPara.SrcFile,"r");//以二进制模式只读输入文件
        ...
        {
            p_g2d_ctx->fd_out = fopen(p_g2d_ctx->mConfigPara.DstFile, "rb");//以二进制模式只读输出文件
        ...
        {
            p_g2d_ctx->fd_out = fopen(p_g2d_ctx->mConfigPara.DstFile, "wb");//以二进制模式只写输出文件
            if (NULL == p_g2d_ctx->fd_out)
            {
                aloge("open out file failed");
                ret = -1;
        ...
        read_len = p_g2d_ctx->src_frm_info.frm_width * p_g2d_ctx->src_frm_info.frm_height;//计算一帧图像的大小
        if(pConfig->mPicFormat == MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420|| pConfig->mPicFormat == MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420)
        {
    ...
    ret = SampleG2d_G2dConvert(p_g2d_ctx);//根据任务类型执行转换
    if (ret < 0)
    {

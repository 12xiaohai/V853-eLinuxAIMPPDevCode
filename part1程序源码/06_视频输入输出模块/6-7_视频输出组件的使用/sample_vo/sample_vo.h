...
typedef struct SampleVOFrameManager {
  ...
  VIDEO_FRAME_INFO_S *(*PrefetchFirstIdleFrame)(
      void *pThiz); //函数指针，从空闲列表中预取第一个空闲帧的操作
  int (*UseFrame)(
      void *pThiz,
      VIDEO_FRAME_INFO_S
          *pFrame); //用于将指定的帧 pFrame
                    //加入到正在使用的列表中，并返回操作结果的整数值。
  int (*ReleaseFrame)(void *pThiz,
                      unsigned int nFrameId); //释放指定 nFrameId 的帧资源
...
typedef struct SampleVOContext {
  ...
  SampleVOConfig
      mConfigPara; //视频输出的配置参数，包括文件路径、尺寸、格式、帧率等信息

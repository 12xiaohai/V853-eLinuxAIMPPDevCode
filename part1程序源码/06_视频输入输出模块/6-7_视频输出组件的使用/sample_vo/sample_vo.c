...
VIDEO_FRAME_INFO_S *SampleVOFrameManager_PrefetchFirstIdleFrame(void *pThiz) {
  ...
  if (!list_empty(&pFrameManager->mIdleList)) {
    pFirstNode =
        list_first_entry(&pFrameManager->mIdleList, SampleVOFrameNode, mList);
...
int SampleVOFrameManager_UseFrame(void *pThiz, VIDEO_FRAME_INFO_S *pFrame) {
  ...
  pthread_mutex_lock(&pFrameManager->mLock);
  SampleVOFrameNode *pFirstNode = list_first_entry_or_null(
      &pFrameManager->mIdleList, SampleVOFrameNode, mList);
  if (pFirstNode) {
    if (&pFirstNode->mFrame == pFrame) {
      list_move_tail(&pFirstNode->mList, &pFrameManager->mUsingList);
    } else {
      aloge("fatal error! node is not match [%p]!=[%p]", pFrame,
            &pFirstNode->mFrame);
      ret = -1;
    }
  } else {
...
int initSampleVOFrameManager(SampleVOFrameManager *pFrameManager, int nFrameNum,
  ...
  for (i = 0; i < nFrameNum; i++) {
    ...
    pNode->mFrame.VFrame.mPhyAddr[1] =
        uPhyAddr + pConfigPara->mPicWidth * pConfigPara->mPicHeight;
    pNode->mFrame.VFrame.mPhyAddr[2] =
        uPhyAddr + pConfigPara->mPicWidth * pConfigPara->mPicHeight +
        pConfigPara->mPicWidth * pConfigPara->mPicHeight / 4;
    pNode->mFrame.VFrame.mpVirAddr[0] = pVirtAddr;
    pNode->mFrame.VFrame.mpVirAddr[1] =
        pVirtAddr + pConfigPara->mPicWidth * pConfigPara->mPicHeight;
    pNode->mFrame.VFrame.mpVirAddr[2] =
        pVirtAddr + pConfigPara->mPicWidth * pConfigPara->mPicHeight +
        pConfigPara->mPicWidth * pConfigPara->mPicHeight / 4;
  ...
  pFrameManager->PrefetchFirstIdleFrame =
      SampleVOFrameManager_PrefetchFirstIdleFrame;
...
int destroySampleVOFrameManager(SampleVOFrameManager *pFrameManager) {
  ...
  if (cnt != pFrameManager->mNodeCnt) {
    aloge("fatal error! frame count is not match [%d]!=[%d]", cnt,
          pFrameManager->mNodeCnt);
  }
  SampleVOFrameNode *pEntry, *pTmp;
  list_for_each_entry_safe(pEntry, pTmp, &pFrameManager->mIdleList, mList) {
    AW_MPI_SYS_MmzFree(pEntry->mFrame.VFrame.mPhyAddr[0],
                       pEntry->mFrame.VFrame.mpVirAddr[0]);
...
static ERRORTYPE SampleVOCallbackWrapper(void *cookie, MPP_CHN_S *pChn,
  ...
  if (MOD_ID_VOU == pChn->mModId) {
    ...
    case MPP_EVENT_RELEASE_VIDEO_BUFFER: {
      ...
      pContext->mFrameManager.ReleaseFrame(&pContext->mFrameManager,
                                           pVideoFrameInfo->mId);
    ...
    case MPP_EVENT_SET_VIDEO_SIZE: {
      ...
      alogd("vo report video display size[%dx%d]", pDisplaySize->Width,
            pDisplaySize->Height);
      break;
    }
...
static ERRORTYPE SampleVO_CLOCKCallbackWrapper(void *cookie, MPP_CHN_S *pChn,
                                               MPP_EVENT_TYPE event,
                                               void *pEventData) {
...
static ERRORTYPE loadSampleVOConfig(SampleVOConfig *pConfig,
  ...
  ptr = (char *)GetConfParaString(&stConfParser, SAMPLE_VO_KEY_YUV_FILE_PATH,
                                  NULL);
  strncpy(pConfig->mYuvFilePath, ptr, MAX_FILE_PATH_SIZE - 1);
  pConfig->mYuvFilePath[MAX_FILE_PATH_SIZE - 1] = '\0';
  pConfig->mPicWidth =
      GetConfParaInt(&stConfParser, SAMPLE_VO_KEY_PIC_WIDTH, 0);
  pConfig->mPicHeight =
      GetConfParaInt(&stConfParser, SAMPLE_VO_KEY_PIC_HEIGHT, 0);
  pConfig->mDisplayWidth =
      GetConfParaInt(&stConfParser, SAMPLE_VO_KEY_DISPLAY_WIDTH, 0);
  pConfig->mDisplayHeight =
      GetConfParaInt(&stConfParser, SAMPLE_VO_KEY_DISPLAY_HEIGHT, 0);

  char *pStrPixelFormat =
      (char *)GetConfParaString(&stConfParser, SAMPLE_VO_KEY_PIC_FORMAT, NULL);
  ...
  char *pStrDispType =
      (char *)GetConfParaString(&stConfParser, SAMPLE_VO_KEY_DISP_TYPE, NULL);
  ...
  pConfig->mFrameRate =
      GetConfParaInt(&stConfParser, SAMPLE_VO_KEY_FRAME_RATE, 0);
...
int main(int argc, char *argv[]) {
  ...
  if (!stContext.mFpYuvFile) {
    aloge("fatal error! can't open yuv file[%s]",
          stContext.mConfigPara.mYuvFilePath);
    result = -1;
    goto _exit;
  }
  /* 1.4 初始化MPP平台 */
...
/* 1.8 设置视频输出配置 */
  ...
  while (stContext.mVOChn < VO_MAX_CHN_NUM) {
    ...
    if (SUCCESS == ret) {
      ...
      aloge("fatal error! create vo channel[%d] ret[0x%x]!", stContext.mVOChn,
            ret);
      break;
    }
  }
  ...
  uint64_t nFrameInterval =
      1000000 / stContext.mConfigPara.mFrameRate; // unit:us
  int nFrameSize;
  ...
  case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420:
    /* 3.1 设置图像帧大小 */
    nFrameSize = stContext.mConfigPara.mPicWidth *
                 stContext.mConfigPara.mPicHeight * 3 / 2;
    break;
  default:
    aloge("fatal error! unknown pixel format[0x%x]",
          stContext.mConfigPara.mPicFormat);
    nFrameSize =
        stContext.mConfigPara.mPicWidth * stContext.mConfigPara.mPicHeight;
    break;
  }
  ...
  while (1) {
    /* 3.1 请求空闲帧*/
    pFrameInfo = stContext.mFrameManager.PrefetchFirstIdleFrame(
        &stContext.mFrameManager);
    if (NULL == pFrameInfo) {
    ...
    AW_MPI_SYS_MmzFlushCache(pFrameInfo->VFrame.mPhyAddr[0],
                             pFrameInfo->VFrame.mpVirAddr[0], nFrameSize);
    /* 3.3 读取文件中的视频帧到空闲帧 */
    nReadLen = fread(pFrameInfo->VFrame.mpVirAddr[0], 1, nFrameSize,
                     stContext.mFpYuvFile);
    ...
    /* 3.5 发送视频帧 */
    ret = AW_MPI_VO_SendFrame(stContext.mVoLayer, stContext.mVOChn, pFrameInfo,
                              0);
    if (ret != SUCCESS) {
      alogd("impossible, send frameId[%d] fail?", pFrameInfo->mId);
      /* 3.6 图像管理器中释放图像 */
      stContext.mFrameManager.ReleaseFrame(&stContext.mFrameManager,
                                           pFrameInfo->mId);
    }
  }
  /* 4、清理工作 */

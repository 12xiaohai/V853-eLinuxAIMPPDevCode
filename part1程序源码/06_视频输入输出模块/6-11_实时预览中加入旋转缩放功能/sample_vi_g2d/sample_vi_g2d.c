...
static int SampleViG2d_G2dConvert_G2dOpen(SampleViG2d_G2dConvert *pThiz)//打开g2d设备节点
{
    int ret = 0;
    pThiz->mG2dFd = open("/dev/g2d", O_RDWR, 0);// 打开设备节点 /dev/g2d，以读写方式打开
    if (pThiz->mG2dFd < 0)
    {
...
static int SampleViG2d_G2dConvert_G2dClose(SampleViG2d_G2dConvert *pThiz)
{
    if(pThiz->mG2dFd >= 0)
    {
        close(pThiz->mG2dFd);//关闭文件描述符 pThiz->mG2dFd
        pThiz->mG2dFd = -1;
    }
    return 0;
}
static int SampleViG2d_G2dConvert_G2dSetConvertParam(SampleViG2d_G2dConvert *pThiz, G2dConvertParam *pConvertParam)//设置转换参数
{
    if(pConvertParam)
    {
...
static int SampleViG2d_G2dConvert_G2dConvertVideoFrame(SampleViG2d_G2dConvert *pThiz, VIDEO_FRAME_INFO_S* pSrcFrameInfo, VIDEO_FRAME_INFO_S* pDstFrameInfo)
{
    ...
    ret = convert_PIXEL_FORMAT_E_to_g2d_fmt_enh(pSrcFrameInfo->VFrame.mPixelFormat, &eSrcFormat);//将输入图像格式转换为g2d中的格式
    if(ret!=SUCCESS)
    {
        aloge("fatal error! src pixel format[0x%x] is invalid!", pSrcFrameInfo->VFrame.mPixelFormat);
        return -1;
    }
    ret = convert_PIXEL_FORMAT_E_to_g2d_fmt_enh(pDstFrameInfo->VFrame.mPixelFormat, &eDstFormat);//将输出图像格式转换为g2d中的格式
    if(ret!=SUCCESS)
    {
...
SampleViG2d_G2dConvert *constructSampleViG2d_G2dConvert()//初始化G2D转换函数指针
{
    ...
    pConvert->mG2dFd = -1;
    pConvert->G2dOpen = SampleViG2d_G2dConvert_G2dOpen;//打开g2d设备节点
    pConvert->G2dClose = SampleViG2d_G2dConvert_G2dClose;//关闭g2d设备节点
    pConvert->G2dSetConvertParam = SampleViG2d_G2dConvert_G2dSetConvertParam;////设置G2D转换参数
    pConvert->G2dConvertVideoFrame = SampleViG2d_G2dConvert_G2dConvertVideoFrame;//G2D图像转换函数
    return pConvert;
}
...
_retry:
    if(!list_empty(&pThiz->mIdleFrameList))//检查空闲帧列表是否为空
    {
        pNode = list_first_entry(&pThiz->mIdleFrameList, VideoFrameInfoNode, mList);//取出第一个空闲帧节点
        pFrameInfo = &pNode->VFrame;
        list_move_tail(&pNode->mList, &pThiz->mFillingFrameList);//移动到填充列表中
    }
    else
    {
        if(0 == nTimeout)//不等待
        ...
        else if(nTimeout < 0)//无限等待
        {
            ...
            {
                pthread_cond_wait(&pThiz->mIdleFrameCond, &pThiz->mLock);//直到有空闲帧可用
            }
        ...
        {
            pThiz->mWaitIdleFrameFlag = true;
            ret = pthread_cond_wait_timeout(&pThiz->mIdleFrameCond, &pThiz->mLock, nTimeout);//在条件变量上等待指定时间
            if(ETIMEDOUT == ret)//如果超时，记录日志并返回 NULL
            {
                alogv("wait output frame timeout[%d]ms, ret[%d]", nTimeout, ret);
                pFrameInfo = NULL;
                pThiz->mWaitIdleFrameFlag = false;
            }
            else if(0 == ret)//如果没有超时且成功唤醒，重新尝试获取空闲帧
            {
                pThiz->mWaitIdleFrameFlag = false;
                goto _retry;
            }
            else //其他错误情况，记录错误日志并返回 NULL
            {
    ...
    pthread_mutex_unlock(&pThiz->mLock);//解锁互斥锁
    return pFrameInfo;
}
//将一个已填充完毕的视频帧从填充列表移动到就绪列表
static int SampleViG2d_VideoFrameManager_FillingFrameDone(SampleViG2d_VideoFrameManager *pThiz, VIDEO_FRAME_INFO_S *pFrame)
{
    int ret = 0;
    int nFindFlag = 0; //是否找到了对应的视频帧
    VideoFrameInfoNode *pNode = NULL, *pTemp;
    pthread_mutex_lock(&pThiz->mLock);//获得互斥锁
    list_for_each_entry_safe(pNode, pTemp, &pThiz->mFillingFrameList, mList)//遍历 mFillingFrameList 列表
    {
        if(pNode->VFrame.mId == pFrame->mId)//查找与 pFrame ID 匹配的节点
        {  
            list_move_tail(&pNode->mList, &pThiz->mReadyFrameList);//如果找到匹配节点，将其从 mFillingFrameList 移动到 mReadyFrameList
            nFindFlag++;
            break;
        }
    }
    if(1 == nFindFlag) //如果成功找到且移动了节点
    {    
        if(pThiz->mWaitReadyFrameFlag)//检查是否有线程在等待准备好帧
        {
            pthread_cond_signal(&pThiz->mReadyFrameCond);//唤醒一个等待的线程
        }
    }
    else
    {
...
static int SampleViG2d_VideoFrameManager_FillingFrameFail(SampleViG2d_VideoFrameManager *pThiz, VIDEO_FRAME_INFO_S *pFrame)
{
    ...
    VideoFrameInfoNode *pNode = NULL, *pTemp;
    pthread_mutex_lock(&pThiz->mLock);
    list_for_each_entry_safe(pNode, pTemp, &pThiz->mFillingFrameList, mList)//遍历填充列表
    {
        if(pNode->VFrame.mId == pFrame->mId)
        {
            list_move(&pNode->mList, &pThiz->mIdleFrameList);//将节点从填充列表移动至空闲列表后
            nFindFlag++;
            break;
        }
    }
    if(1 == nFindFlag)//如果成功找到且移动了节点
    {
        if(pThiz->mWaitIdleFrameFlag)//检查是否有线程在等待空闲帧
        {
            pthread_cond_signal(&pThiz->mIdleFrameCond);// 唤醒一个等待的线程
        }
    }
    else
    {
...
static VIDEO_FRAME_INFO_S* SampleViG2d_VideoFrameManager_GetReadyFrame(SampleViG2d_VideoFrameManager *pThiz, int nTimeout)
{
    ...
    VideoFrameInfoNode* pNode = NULL;
    VIDEO_FRAME_INFO_S *pFrameInfo = NULL;
    pthread_mutex_lock(&pThiz->mLock);//锁定互斥锁
_retry:
    if(!list_empty(&pThiz->mReadyFrameList))//检查就绪帧列表是否为空
    {
        pNode = list_first_entry(&pThiz->mReadyFrameList, VideoFrameInfoNode, mList);//取出第一个节点
        pFrameInfo = &pNode->VFrame;//将节点中的视频帧信息指针赋值
        list_move_tail(&pNode->mList, &pThiz->mUsingFrameList);//将该节点从就绪帧列表移动到正在使用的帧列表
    }
    else //处理超时情况
    {
        if(0 == nTimeout)//不等待，未获取到可用的视频帧
        {
            pFrameInfo = NULL;
        }
        else if(nTimeout < 0)//无限等待
        {
            ...
            while(list_empty(&pThiz->mReadyFrameList))//循环等待就绪帧列表不为空
            {
                pthread_cond_wait(&pThiz->mReadyFrameCond, &pThiz->mLock);//等待就绪帧条件变量
            }
        ...
        else //等待特定秒
        {
            ...
            if(ETIMEDOUT == ret) //如果等待超时
            {
                alogv("wait output frame timeout[%d]ms, ret[%d]", nTimeout, ret);
                pFrameInfo = NULL;
                pThiz->mWaitReadyFrameFlag = false;
            }
            else if(0 == ret) //如果等待成功
            {
                pThiz->mWaitReadyFrameFlag = false;
                goto _retry;
            }
...
{
    int ret = 0;
    int nFindFlag = 0;// 标志变量，用于记录找到匹配的帧
    VideoFrameInfoNode *pNode = NULL, *pTemp;
    pthread_mutex_lock(&pThiz->mLock);
    list_for_each_entry_safe(pNode, pTemp, &pThiz->mUsingFrameList, mList)//循环宏遍历 mUsingFrameList 链表中的节点
    {
        if(pNode->VFrame.mId == pFrame->mId)
        {
            list_move_tail(&pNode->mList, &pThiz->mIdleFrameList);//将节点从使用中列表移动到空闲帧列表
            nFindFlag++;
            break;
        }
    }
    if(1 == nFindFlag)
    {
        ...
        {
            pthread_cond_signal(&pThiz->mIdleFrameCond);//发送信号给等待空闲帧的线程，通知有空闲帧可用。
        }
    }
    else
    {
...
{
    int cnt = 0; //计数空闲帧的整数变量
    struct list_head *pList;
    pthread_mutex_lock(&pThiz->mLock);
    list_for_each(pList, &pThiz->mIdleFrameList)//循环宏遍历 mIdleFrameList 链表中的每个节点
    {
        cnt++;
    }
    pthread_mutex_unlock(&pThiz->mLock);
...
SampleViG2d_VideoFrameManager *constructSampleViG2d_VideoFrameManager(int nFrameNum, PIXEL_FORMAT_E nPixelFormat, int nBufWidth, int nBufHeight)
{
    ...
    ret = pthread_mutex_init(&pManager->mLock, NULL); //初始化互斥锁
    if(ret != 0)
    {
    ...
    pthread_condattr_setclock(&condAttr, CLOCK_MONOTONIC);//时钟设为 CLOCK_MONOTONIC
    ret = pthread_cond_init(&pManager->mIdleFrameCond, &condAttr);//初始化空闲帧条件变量
    if(ret != 0)
    {
        aloge("pthread cond init fail!");
    }
    ret = pthread_cond_init(&pManager->mReadyFrameCond, &condAttr);//初始化就绪帧条件变量
    if(ret != 0)
    {
        aloge("pthread cond init fail!");
    }
    ...
    pManager->GetIdleFrame          = SampleViG2d_VideoFrameManager_GetIdleFrame;//获取空闲的视频帧
    pManager->FillingFrameDone      = SampleViG2d_VideoFrameManager_FillingFrameDone;//将一个已填充完毕的视频帧从填充列表移动到就绪列表
    pManager->FillingFrameFail      = SampleViG2d_VideoFrameManager_FillingFrameFail; //处理填充视频帧失败的情况
    pManager->GetReadyFrame         = SampleViG2d_VideoFrameManager_GetReadyFrame; //从就绪帧列表中获取一个可用的视频帧
    pManager->UsingFrameDone        = SampleViG2d_VideoFrameManager_UsingFrameDone;//从正在使用的帧移动到空闲帧列表
    pManager->QueryIdleFrameNum     = SampleViG2d_VideoFrameManager_QueryIdleFrameNum;//查询空闲帧数量
    pManager->QueryFillingFrameNum  = SampleViG2d_VideoFrameManager_QueryFillingFrameNum;//查询正在填充帧数量
    pManager->QueryReadyFrameNum    = SampleViG2d_VideoFrameManager_QueryReadyFrameNum;//查询就绪帧数量
    pManager->QueryUsingFrameNum    = SampleViG2d_VideoFrameManager_QueryUsingFrameNum;//查询正在使用中帧数量
    return pManager;
...
int main(int argc, char *argv[])
{
    ...
    pContext->mViCapCtx.mViAttr.nbufs = 5;
    ...
    AW_MPIVO_Enable(pContext->mVoDisplayCtx.mVoDev);
    ...
    AW_MPI_VO_SetPubAttr(pContext->mVoDisplayCtx.mVoDev, &pContext_->mVoDisplayCtx.mPubAttr);

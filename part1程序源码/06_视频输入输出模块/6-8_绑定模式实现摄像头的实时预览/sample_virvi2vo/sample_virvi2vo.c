...
static ERRORTYPE SampleVIRVI2VO_VOCallbackWrapper(void *cookie, MPP_CHN_S *pChn,
                                                  MPP_EVENT_TYPE event,
                                                  void *pEventData) {
  ERRORTYPE ret = SUCCESS;
  SampleVIRVI2VOContext *pContext = (SampleVIRVI2VOContext *)cookie;
  if (MOD_ID_VOU == pChn->mModId) {
    alogd("VO callback: VO Layer[%d] chn[%d] event:%d", pChn->mDevId,
          pChn->mChnId, event);
    switch (event) {
    case MPP_EVENT_RELEASE_VIDEO_BUFFER: {
      VIDEO_FRAME_INFO_S *pFrameInfo = (VIDEO_FRAME_INFO_S *)pEventData;
      aloge("vo layer[%d] release frame id[0x%x]!", pChn->mDevId,
            pFrameInfo->mId);
      break;
    }
    case MPP_EVENT_SET_VIDEO_SIZE: {
      SIZE_S *pDisplaySize = (SIZE_S *)pEventData;
      alogd("vo layer[%d] report video display size[%dx%d]", pChn->mDevId,
            pDisplaySize->Width, pDisplaySize->Height);
      break;
    }
...
static ERRORTYPE SampleVIRVI2VO_CLOCKCallbackWrapper(void *cookie,
                                                     MPP_CHN_S *pChn,
                                                     MPP_EVENT_TYPE event,
                                                     void *pEventData) {
...
static int ParseCmdLine(int argc, char **argv,
                        SampleVIRVI2VOCmdLineParam *pCmdLinePara) {
  ...
  while (i < argc) {
    if (!strcmp(argv[i], "-path")) {
      ...
      if (strlen(argv[i]) >= MAX_FILE_PATH_SIZE) {
        aloge("fatal error! file path[%s] too long: [%d]>=[%d]!", argv[i],
              strlen(argv[i]), MAX_FILE_PATH_SIZE);
      }
      ...
      alogd("ignore invalid CmdLine param:[%s], type -h to get how to set "
            "parameter!",
            argv[i]);
    }
    i++;
...
static void LoadVIPP2VOConfig(int idx, VIPP2VOConfig *pVIPP2VOConfig,
                              CONFPARSER_S *pConfParser) {
  if (0 == idx) {
    pVIPP2VOConfig->mIspDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_ISP_DEV, 0);
    pVIPP2VOConfig->mVippDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_VIPP_DEV, 0);
    pVIPP2VOConfig->mCaptureWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_WIDTH, 0);
    pVIPP2VOConfig->mCaptureHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_HEIGHT, 0);
    pVIPP2VOConfig->mDisplayX =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_X, 0);
    pVIPP2VOConfig->mDisplayY =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_Y, 0);
    pVIPP2VOConfig->mDisplayWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_WIDTH, 0);
    pVIPP2VOConfig->mDisplayHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_HEIGHT, 0);
    pVIPP2VOConfig->mLayerNum =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_LAYER_NUM, 0);
  } else if (1 == idx) {
    pVIPP2VOConfig->mIspDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_ISP_DEV2, 0);
    pVIPP2VOConfig->mVippDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_VIPP_DEV2, 0);
    pVIPP2VOConfig->mCaptureWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_WIDTH2, 0);
    pVIPP2VOConfig->mCaptureHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_HEIGHT2, 0);
    pVIPP2VOConfig->mDisplayX =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_X2, 0);
    pVIPP2VOConfig->mDisplayY =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_Y2, 0);
    pVIPP2VOConfig->mDisplayWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_WIDTH2, 0);
    pVIPP2VOConfig->mDisplayHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_HEIGHT2, 0);
    pVIPP2VOConfig->mLayerNum =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_LAYER_NUM2, 0);
  } else if (2 == idx) {
    pVIPP2VOConfig->mIspDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_ISP_DEV3, 0);
    pVIPP2VOConfig->mVippDev =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_VIPP_DEV3, 0);
    pVIPP2VOConfig->mCaptureWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_WIDTH3, 0);
    pVIPP2VOConfig->mCaptureHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_CAPTURE_HEIGHT3, 0);
    pVIPP2VOConfig->mDisplayX =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_X3, 0);
    pVIPP2VOConfig->mDisplayY =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_Y3, 0);
    pVIPP2VOConfig->mDisplayWidth =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_WIDTH3, 0);
    pVIPP2VOConfig->mDisplayHeight =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_DISPLAY_HEIGHT3, 0);
    pVIPP2VOConfig->mLayerNum =
        GetConfParaInt(pConfParser, SAMPLE_VIRVI2VO_KEY_LAYER_NUM3, 0);
  } else {
    aloge("fatal error! need add vipp[%d] config!", idx);
  }
}

static ERRORTYPE loadSampleVIRVI2VOConfig(SampleVIRVI2VOConfig *pConfig,
  ...
  pConfig->mbAddUILayer =
      (BOOL)GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_ADD_UI_LAYER, 0);
  pConfig->mTestUILayer =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_UI_TEST_LAYER, 0);
  pConfig->mUIDisplayWidth =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_UI_DISPLAY_WIDTH, 0);
  pConfig->mUIDisplayHeight =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_UI_DISPLAY_HEIGHT, 0);

  char *pStrPixelFormat = (char *)GetConfParaString(
      &stConfParser, SAMPLE_VIRVI2VO_KEY_PIC_FORMAT, NULL);
  ...
  else if (!strcmp(pStrPixelFormat, "aw_afbc")) {
    pConfig->mPicFormat = MM_PIXEL_FORMAT_YUV_AW_AFBC;
  ...
  } else if (!strcmp(pStrPixelFormat, "srggb10")) {
    pConfig->mPicFormat = MM_PIXEL_FORMAT_RAW_SRGGB10;
  } else {
    aloge("fatal error! conf file pic_format is [%s]?", pStrPixelFormat);
    pConfig->mPicFormat = MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420;
  }
  char *pStrDispType = (char *)GetConfParaString(
      &stConfParser, SAMPLE_VIRVI2VO_KEY_DISP_TYPE, NULL);
  ...
  pConfig->mFrameRate =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_FRAME_RATE, 0);
  pConfig->mTestDuration =
      GetConfParaInt(&stConfParser, SAMPLE_VIRVI2VO_KEY_TEST_DURATION, 0);

  for (i = 0; i < VIPP2VO_NUM; i++) {
    alogd("vipp[%d]: captureSize[%dx%d], displayArea[%d,%d,%dx%d]",
          pConfig->mVIPP2VOConfigArray[i].mVippDev,
          pConfig->mVIPP2VOConfigArray[i].mCaptureWidth,
          pConfig->mVIPP2VOConfigArray[i].mCaptureHeight,
          pConfig->mVIPP2VOConfigArray[i].mDisplayX,
          pConfig->mVIPP2VOConfigArray[i].mDisplayY,
          pConfig->mVIPP2VOConfigArray[i].mDisplayWidth,
          pConfig->mVIPP2VOConfigArray[i].mDisplayHeight);
  }
  alogd("dispSync[%d], dispType[%d], frameRate[%d], testDuration[%d]",
        pConfig->mDispSync, pConfig->mDispType, pConfig->mFrameRate,
        pConfig->mTestDuration);
...
static ERRORTYPE SampleVirvi2Vo_MPPCallback(void *cookie, MPP_CHN_S *pChn,
                                            MPP_EVENT_TYPE event,
                                            void *pEventData) {
  SampleVIRVI2VOContext *pContext = (SampleVIRVI2VOContext *)cookie;
...
#if ISP_RUN
  // if enable CONFIG_ENABLE_SENSOR_FLIP_OPTION, flip operations must be after
  // stream_on.
  AW_MPI_VI_SetVippMirror(pLinkInfo->mVIDev, 0); // 0,1  水平镜像
  AW_MPI_VI_SetVippFlip(pLinkInfo->mVIDev, 0);   // 0,1 垂直镜像
#endif
  ...
  */
  ...
  while (pLinkInfo->mVOChn < VO_MAX_CHN_NUM) {
    ...
    if (SUCCESS == eRet) {
      ...
      aloge("fatal error! create vo channel[%d] ret[0x%x]!", pLinkInfo->mVOChn,
            eRet);
      break;
    }
  }
  ...
  /* bind clock,vo, viChn
  (because frame information has 'pts', there is no need to bind clock channel
  now)
  */
...
int DrawFb(SampleVIRVI2VOContext *pContext) {
  ...
  if (-1 == ioctl(pContext->mFbDev.mFbFd, FBIOGET_VSCREENINFO,
                  &pContext->mFbDev.mFbVar)) {
    aloge("ioctl FBIOGET_VSCREENINFO fail");
    return -1;
  }
  alogd("fb_var_screeninfo: [%d,%d], [%d,%d], [%d,%d], bitsPerPixel[%d], "
        "grayscale[%d]",
        pContext->mFbDev.mFbVar.xres, pContext->mFbDev.mFbVar.yres,
        pContext->mFbDev.mFbVar.xres_virtual,
        pContext->mFbDev.mFbVar.yres_virtual, pContext->mFbDev.mFbVar.xoffset,
        pContext->mFbDev.mFbVar.yoffset, pContext->mFbDev.mFbVar.bits_per_pixel,
        pContext->mFbDev.mFbVar.grayscale);
  if (-1 == ioctl(pContext->mFbDev.mFbFd, FBIOGET_FSCREENINFO,
                  &pContext->mFbDev.mFbFix)) {
    aloge("ioctl FBIOGET_FSCREENINFO fail");
    return -1;
  }
  alogd("fb_fix_screeninfo: smem_start[0x%x], smem_len[%ld], pageSize[%d]",
        pContext->mFbDev.mFbFix.smem_start, pContext->mFbDev.mFbFix.smem_len,
        getpagesize());
  // map physics address to virtual address
  pContext->mFbDev.mFbMemOffset =
      pContext->mFbDev.mFbFix.smem_start -
      (pContext->mFbDev.mFbFix.smem_start & ~(getpagesize() - 1));
  pContext->mFbDev.mFbMem = mmap(
      NULL, pContext->mFbDev.mFbFix.smem_len + pContext->mFbDev.mFbMemOffset,
      PROT_READ | PROT_WRITE, MAP_SHARED, pContext->mFbDev.mFbFd, 0);
  alogd("mmap: mem:%p, offset:%ld", pContext->mFbDev.mFbMem,
        pContext->mFbDev.mFbMemOffset);
  if (MAP_FAILED == pContext->mFbDev.mFbMem) {
    aloge("mmap error! mem:%p offset:%ld", pContext->mFbDev.mFbMem,
          pContext->mFbDev.mFbMemOffset);
    return -1;
  }
  ...
  */
  char *mem_start =
      (char *)pContext->mFbDev.mFbMem + pContext->mFbDev.mFbMemOffset;
  ...
  int *add =
      (int *)((char *)pContext->mFbDev.mFbMem + pContext->mFbDev.mFbMemOffset);
  int i;
  for (i = 0; i < w * h; i++) {
    if (i < w * h / 2) {
      *add = 0x7F00FF00;
  ...
  //    void *args[2];
  //    void *dirty_rect_vir_addr_begin = (mem_start +  pitch*y +
  //    bytes_per_pixel*x); void *dirty_rect_vir_addr_end  = (mem_start + pitch
  //    * (y+ h)); args[0] = dirty_rect_vir_addr_begin; args[1] =
  //    dirty_rect_vir_addr_end; ret = ioctl(pContext->mFbDev.nFbFd,
  //    FBIO_CACHE_SYNC, args); if(ret != 0)
  //    {
  ...
  ret =
      ioctl(pContext->mFbDev.mFbFd, FBIOPAN_DISPLAY, &pContext->mFbDev.mFbVar);
  if (ret != 0) {
    aloge("fatal error! FBIOPAN DISPLAY fail[%d]", ret);
  }

  munmap(pContext->mFbDev.mFbMem,
         pContext->mFbDev.mFbFix.smem_len + pContext->mFbDev.mFbMemOffset);
...
int CreateTestUILayer(SampleVIRVI2VOContext *pContext) {
  ...
  AW_MPI_VO_GetVideoLayerAttr(pContext->mTestUILayer,
                              &pContext->mTestUILayerAttr);
  pContext->mTestUILayerAttr.stDispRect.X = 0;
  pContext->mTestUILayerAttr.stDispRect.Y = 0;
  pContext->mTestUILayerAttr.stDispRect.Width =
      pContext->mConfigPara.mUIDisplayWidth;
  pContext->mTestUILayerAttr.stDispRect.Height =
      pContext->mConfigPara.mUIDisplayHeight;
  AW_MPI_VO_SetVideoLayerAttr(pContext->mTestUILayer,
                              &pContext->mTestUILayerAttr);
  ...
  case MM_PIXEL_FORMAT_RGB_8888:
    nFrameBufLen = pContext->mConfigPara.mUIDisplayWidth *
                   pContext->mConfigPara.mUIDisplayHeight * 4;
    break;
  case MM_PIXEL_FORMAT_RGB_1555:
  case MM_PIXEL_FORMAT_RGB_565:
    nFrameBufLen = pContext->mConfigPara.mUIDisplayWidth *
                   pContext->mConfigPara.mUIDisplayHeight * 2;
    break;
  default:
  ...
  pContext->mTestUIFrame.VFrame.mHeight =
      pContext->mConfigPara.mUIDisplayHeight;
  ...
  pContext->mTestUIFrame.VFrame.mOffsetBottom =
      pContext->mTestUIFrame.VFrame.mHeight;
  pContext->mTestUIFrame.VFrame.mOffsetLeft = 0;
  pContext->mTestUIFrame.VFrame.mOffsetRight =
      pContext->mTestUIFrame.VFrame.mWidth;
  ...
  while (pContext->mTestVOChn < VO_MAX_CHN_NUM) {
    ...
    if (SUCCESS == ret) {
      ...
      aloge("fatal error! create vo channel[%d] ret[0x%x]!",
            pContext->mTestVOChn, ret);
      break;
    }
  }
  ...
  cbInfo.cookie = (void *)pContext;
  cbInfo.callback = (MPPCallbackFuncType)&SampleVIRVI2VO_VOCallbackWrapper;
  AW_MPI_VO_RegisterCallback(pContext->mTestUILayer, pContext->mTestVOChn,
                             &cbInfo);
  ...
  ret = AW_MPI_VO_SendFrame(pContext->mTestUILayer, pContext->mTestVOChn,
                            &pContext->mTestUIFrame, 0);
  if (ret != SUCCESS) {
...
_err1:
  ret = AW_MPI_SYS_MmzFree(pContext->mTestUIFrame.VFrame.mPhyAddr[0],
                           pContext->mTestUIFrame.VFrame.mpVirAddr[0]);
  if (ret != SUCCESS) {
...
int main(int argc __attribute__((__unused__)),
...
#endif
  if (loadSampleVIRVI2VOConfig(&stContext.mConfigPara, pConfigFilePath) !=
      SUCCESS) {
...
#if ISP_RUN
    result = AW_MPI_ISP_SwitchIspConfig(
        stContext.mConfigPara.mVIPP2VOConfigArray[i].mIspDev,
        stContext.mConfigPara
            .mIspWdrSetting); //切换图像效果（彩色、黑白），切换编译进固件的
                              // ISP 效果参数。
    if (result != SUCCESS) {
      aloge("fatal error! isp switch wdr[%d] fail[%d]",
            stContext.mConfigPara.mIspWdrSetting, result);
    }
#endif
  }
  ...
  if (stContext.mConfigPara.mTestDuration > 0) {
    cdx_sem_down_timedwait(&stContext.mSemExit,
                           stContext.mConfigPara.mTestDuration * 1000);
  } else {
  ...
  //    if(result != 0)
  //    {
  //        aloge("fatal error! isp[%d] set save ctx wrong[%d]!",
  //        stContext.mIspDev, result);
  //    }
  //    else
  //    {

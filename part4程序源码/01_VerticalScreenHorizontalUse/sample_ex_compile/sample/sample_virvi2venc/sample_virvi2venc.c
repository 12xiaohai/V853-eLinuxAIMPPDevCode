...
static ERRORTYPE loadConfigPara(SAMPLE_VIRVI2VENC_S *pContext, const char *conf_path)
{
    ...
    if (conf_path != NULL)
    {
        ...
        if (ptr != NULL)
        {
            ...
            {
                pContext->mConfigPara.srcPixFmt = MM_PIXEL_FORMAT_YUV_AW_AFBC;
            }
            ...
            // {
            //     pContext->mConfigPara.srcPixFmt = MM_PIXEL_FORMAT_YVYU_AW_PACKAGE_422;
            // }
            else
            {
        ...
        if (pContext->mConfigPara.mReqIDREnable)
        {
            if (0 == count%100)
            {
                ...
                if (ret)
                    ...
                    aloge("VeChn[%d] req IDR", nVencChn);
            }
        }

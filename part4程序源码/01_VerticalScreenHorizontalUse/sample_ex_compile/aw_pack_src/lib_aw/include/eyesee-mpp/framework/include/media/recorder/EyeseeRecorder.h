...
struct SinkParam
{
    ...
    char* mOutputPath;
    int   mFallocateLen;
    int   mMaxDurationMs;   //>0: valid, <=0:invalid, use global value.
...
private:
    ...
    {
    public:
        CameraProxyListener(EyeseeRecorder *recorder, VI_DEV Vipp);
    ...
    //VideoEncodeProductMode mVideoPDMode;        // product mode:ipc sdv/cdr .etc
...
public:
     ...
     * @param channelId camera/ISE channel id.
     */
    status_t setCameraProxy(CameraRecordingProxy *pCameraProxy, VI_DEV Vipp);
    ...
    //status_t setVideoEncodingProductMode(VideoEncodeProductMode pdMode);
    //status_t setSensorType(eSensorType eType);
 ...
 */
    ...
    status_t vencprepare(int VencId);

    status_t setVencParameters(int VencId, VencParameters *pVencParam);

    status_t bindVeVipp(int VencId, VI_DEV Vipp);
...
public:
    ...
    void dataCallbackTimestamp(const VIDEO_FRAME_BUFFER_S *pCameraFrameInfo, VI_DEV Vipp);
     ...
     * @param muxerId.
     */
    status_t removeOutputSink(int muxerId);
    ...
    status_t attachRegionToVenc(int VencId, RGN_HANDLE Handle, const RGN_CHN_ATTR_S *pstChnAttr);
    status_t detachRegionFromVenc(int VencId, RGN_HANDLE Handle);
    status_t setRegionDisplayAttrOfVenc(int VencId, RGN_HANDLE Handle, const RGN_CHN_ATTR_S *pstChnAttr);
    status_t getRegionDisplayAttrOfVenc(int VencId, RGN_HANDLE Handle, RGN_CHN_ATTR_S *pstChnAttr);

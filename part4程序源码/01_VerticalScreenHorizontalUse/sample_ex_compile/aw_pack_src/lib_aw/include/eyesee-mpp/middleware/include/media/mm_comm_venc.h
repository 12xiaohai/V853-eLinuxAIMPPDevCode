...
typedef enum VENC_REF_FRAME_LBC_MODE_E
{
    VENC_REF_FRAME_LBC_MODE_DEFAULT  = 0,  /* default: 1.5x. */
    VENC_REF_FRAME_LBC_MODE_1_5X     = 1,
    VENC_REF_FRAME_LBC_MODE_2_0X     = 2,
    VENC_REF_FRAME_LBC_MODE_2_5X     = 3,
    VENC_REF_FRAME_LBC_MODE_NO_LOSSY = 4,
} VENC_REF_FRAME_LBC_MODE_E;
...
{
    BOOL mbEncppEnable;                           /*enable Encpp sharp, FALSE:disable, TRUE:enable, default value:FALSE. If the data is not from isp, set FALSE.*/
    //unsigned int mEncppSharpAttenCoefPer;         /*Encpp sharp attenuation percentage coefficient, the default is 100%, no attenuation.*/
}VENC_ENCPP_ATTR_S;
...
typedef struct VENC_PARAM_VUI_BITSTREAM_RESTRIC_S {
    unsigned char  bitstream_restriction_flag ;            /* default value: 0. see the protocol for the meaning. {0,1} */
} VENC_PARAM_VUI_BITSTREAM_RESTRIC_S;

typedef struct VENC_PARAM_H264_VUI_S
{
	VENC_PARAM_VUI_ASPECT_RATIO_S       VuiAspectRatio;      /* Currently not supported. */
	VENC_PARAM_VUI_H264_TIME_INFO_S     VuiTimeInfo;
	VENC_PARAM_VUI_VIDEO_SIGNAL_S       VuiVideoSignal;      /* Currently not supported. */
	VENC_PARAM_VUI_BITSTREAM_RESTRIC_S  VuiBitstreamRestric; /* Currently not supported. */
}VENC_PARAM_H264_VUI_S;

typedef struct VENC_PARAM_VUI_H265_TIME_INFO_S {
    unsigned int timing_info_present_flag;                 /* default value: 0. If 1, timing info belows will be encoded into vui. {0,1} */
    unsigned int num_units_in_tick;                        /* default value: n/a. [0,4294967295]; see the H.265 protocol for the meaning. */
    unsigned int time_scale;                               /* default value: n/a. (0,4294967295]; see the H.265 protocol for the meaning */
    unsigned int num_ticks_poc_diff_one_minus1;            /* default value: n/a. (0,4294967294]; see the H.265 protocol for the meaning */
} VENC_PARAM_VUI_H265_TIME_INFO_S;

typedef struct VENC_PARAM_H265_VUI_S {
    VENC_PARAM_VUI_ASPECT_RATIO_S       VuiAspectRatio;      /* Currently not supported. */
    VENC_PARAM_VUI_H265_TIME_INFO_S     VuiTimeInfo;
    VENC_PARAM_VUI_VIDEO_SIGNAL_S       VuiVideoSignal;      /* Currently not supported. */
    VENC_PARAM_VUI_BITSTREAM_RESTRIC_S  VuiBitstreamRestric; /* Currently not supported. */
